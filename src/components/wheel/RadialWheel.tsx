...
/*
 * Gap between the outer edge of the primary wheel and the
 * nested (submenu) ring. The nested ring shares the SAME
 * center as the primary wheel — it is just a bigger circle:
 *
 *                    ●  ChatGPT
 *
 *              ●              ●
 ...
 *                 (center) ── AI
 *
 * nestedRadius = radius + NESTED_RING_GAP
 */
const NESTED_RING_GAP = 60;

/*
 * Maximum angular spread of the child ring.
 */
const MAX_CHILD_ARC = Math.PI * 0.5;

/*
 * Minimum angular spread.
 */
const MIN_CHILD_ARC = Math.PI * 0.3;

/*
 * Size used when checking whether the pointer
 * is close enough to a child item.
 */
const CHILD_HIT_RADIUS = 60;
...
export const RadialWheel: React.FC<RadialWheelProps> = ({
  ...
  /* ==========================================================
     ...
     Children are positioned relative to the wheel CENTER,
     on a concentric ring at a larger radius than the
     primary wheel:

         nestedRadius = radius + NESTED_RING_GAP

         childX =
           cos(childAngle) * nestedRadius

         childY =
           sin(childAngle) * nestedRadius

     The ring of children is centered on the same angular
     direction as the parent item (baseAngle, below), so the
     submenu still reads as "belonging" to that item — it
     just sits on its own concentric ring instead of being
     clustered next to the parent icon.
     ========================================================== */
  ...
  const childSectors =
    useMemo(() => {
      ...
      /*
       * --------------------------------------------------------
       * Nested ring radius — shares the wheel's center,
       * just a larger circle.
       * --------------------------------------------------------
       */

      const nestedRadius =
        radius + NESTED_RING_GAP;
      ...
      return activeChildren.map(
        (child, index) => {
          ...
          /*
           * Child is positioned on the concentric
           * nested ring, from the wheel center.
           */
  ...
  /* ==========================================================
   OUTER CHILD HOVER WEDGE

   IMPORTANT:
   - Only appears when a CHILD is hovered.
   - Never highlights a parent that has no children.
   - Uses the exact angle of the hovered child.
   - Highlights ONE child at a time.
   ========================================================== */

const outerWedgePath = useMemo(() => {
  /*
   * No child selected = no outer highlight.
   */
  if (
    !activeChildId ||
  ...
  /*
   * Find the currently hovered child.
   */
  const activeChildSector =
    childSectors.find(
      (sector) =>
        sector.child.id ===
        activeChildId,
  ...
  /*
   * Start just outside the primary wheel.
   */
  const innerRadius =
    radius + 3;
  ...
  /*
   * ----------------------------------------------------------
   * Calculate a SMALL angular sector around the child.
   *
   * We intentionally do NOT use the complete parent sector.
   *
   * This prevents:
   *
   *      child 1
   *      child 2
   *
   * from being highlighted together.
   * ----------------------------------------------------------
   */
  ...
  /*
   * If there are multiple children, calculate
   * the available spacing between them and make
   * the highlight slightly smaller than that spacing.
   */
  if (
    ...
    const currentIndex =
      sortedAngles.findIndex(
        (angle) =>
          Math.abs(
            angle -
              activeChildSector.angle,
          ) < 0.0001,
      );
    ...
    if (
      ...
      if (
        ...
        /*
         * 38% of the distance between
         * child centers.
         *
         * This guarantees a visible gap
         * between adjacent highlighted areas.
         */
        halfAngle =
          Math.min(
            Math.PI / 8,
  ...
  const resolvePointer =
    useCallback(
      (
        ...
        /*
         * Allow pointer to reach the nested ring, which
         * sits at radius + NESTED_RING_GAP from center.
         */
        const outsideWheel =
          distance >
          radius +
